...
inline bool sortByVoteNumber(const std::pair<int, int> a, const std::pair<int, int> b) {
...
inline void pcl::registration::GRORInitialAlignment<PointSource, PointTarget, Scalar>::computeTransformation(PointCloudSource & output, const Eigen::Matrix4f & guess)
{
	...
	fitness_score_ = calculateMSAC(final_transformation_);
}
template<typename PointSource, typename PointTarget, typename Scalar>
inline float pcl::registration::GRORInitialAlignment<PointSource, PointTarget, Scalar>::calculateMSAC(Eigen::Matrix4f trans)
{
	//pcl::IndicesPtr target_indices_;
	//target_indices_.reset(new std::vector <int>(static_cast <int> (iss_target_->size())));
	//int index = 0;
	//for (int& target_index : *target_indices_)
	//	target_index = index++;
	float lambda_ = 0;
	//Eigen::Vector4f pt_min, pt_max;
	////iss_target_->is_dense = false;
	//pcl::getMinMax3D(*iss_target_, pt_min, pt_max);
	//float diameter_ = (pt_max - pt_min).norm();

	//float mean_dist = pcl::getMeanPointDensity <pcl::PointXYZ>(iss_target_, *target_indices_, 0.05f * diameter_);
	//delta_ *= mean_dist;

	float max_inlier_dist_sqr_ = powf(delta_ * 2.f, 2.f);
	...
	pcl::transformPointCloud(*input_, source_transformed, final_transformation_);
	...
	for (auto it = source_transformed.begin(), it_e = source_transformed.end(); it != it_e; ++it)
	{
	...
	//fitness_score_ = fitness_score_temp;

	return fitness_score_temp;
}
